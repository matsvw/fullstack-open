...
const createModelLoader = Model =>
  new DataLoader(
    async ids => {
      const idColumns = lodash.isArray(Model.idColumn)
        ? Model.idColumn
        : [Model.idColumn];

      const camelCasedIdColumns = idColumns.map(id => lodash.camelCase(id));
      ...
      return ids.map(
        id =>
          lodash.find(
            results,
            lodash.zipObject(camelCasedIdColumns, lodash.isArray(id) ? id : [id]),
          ) || null,
      );
    },
    {
...
const createRepositoryRatingAverageLoader = () =>
  new DataLoader(async repositoryIds => {
    const reviews = await Review.query()
      ...
      .select('repositoryId');
    //console.log("Reviews for ratings: ", reviews);
...
const createRepositoryReviewCountLoader = () =>
  new DataLoader(async repositoryIds => {
    ...
    //console.log("Reviews for reviews: ", reviews);
    return repositoryIds.map(id => {
      const review = reviews.find(({ repositoryId }) => repositoryId === id);
